    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(this, &MainWindow::dataReady, this, &MainWindow::draw);
...
void MainWindow::draw(QVector<double> x,QVector<double> y ){
    ...
    spline->setName("Первые 1000 точек");

    for (auto index = 0; index < x.length(); index++){
        spline->append(x[index],y[index]);
    };
    QChart *chart = new QChart();
    chart->addSeries(spline);
    chart->createDefaultAxes();
    chart->setTitle("QSplineSeries");
...
void MainWindow::on_pb_start_clicked()
{
    ...
    auto findMax = [&](QVector<double> res){
        int samplesPerSecond = 1000;
                                                ...
                                                auto val = res.mid(0, samplesPerSecond);
                                                QVector<double> timeValues(samplesPerSecond);
                                                for (int i = 0; i < samplesPerSecond; ++i) {
                                                    timeValues[i] = i / 1000.0;
                                                }
                                                emit dataReady(timeValues, val);
